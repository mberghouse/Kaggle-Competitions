...
class Config:
    seed = 42 
    image_transform = transforms.Resize((340,340))  
    batch_size = 50
    num_epochs = 10
    num_folds = 5
...
train_idx, test_idx = train_test_split(total_idx, test_size=0.15)
...
train_dataloader = DataLoader(train_dataset, batch_size=36, shuffle=True,  num_workers=0, drop_last=False)
test_dataset = CustomDataset(train_feats.iloc[test_idx], test_data)
test_dataloader = DataLoader(test_dataset, batch_size=36, shuffle=False,  num_workers=0, drop_last=False)
torch.cuda.empty_cache()
fold=3
weights_path = 'C:\\Users\\marcb\\OneDrive\\Desktop\\Kaggle_Competitions\\Kaggle-Competitions\\HMS-harmful-brain-activity/efficientnet_b0_fold3.pth'
# Initialize EfficientNet-B0 model with pretrained weights
model = timm.create_model('regnety_032', pretrained=False, drop_rate=.2,drop_path_rate=0.1,output_stride=16, num_classes=6, in_chans=1)
model = redo_classifier(model)
model.load_state_dict(torch.load(weights_path))
model.to(device)

optimizer = optim.AdamW(model.parameters(), betas=(0.5, 0.995),lr=0.00002, weight_decay=0.2)
